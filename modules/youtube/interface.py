...
module_information = ModuleInformation(
    ...
    module_supported_modes=ModuleModes.download | ModuleModes.playlist | ModuleModes.lyrics,
    global_settings={},
    session_settings={},
    flags=ModuleFlags.hidden,
    netlocation_constant=['youtube.com', 'youtu.be']
)
...
class ModuleInterface:
    ...
    def _run_yt_dlp_json(self, url_or_query, ytdlp_args=None):
        # Try python yt_dlp first, fall back to CLI
        try:
    ...
    def search(self, media_type, query, limit=10, track_info=None):
        ...
        results = []
        for e in entries:
            sr = SearchResult(result_id=e.get('id') or e.get('url'), name=e.get('title'), artists=[e.get('uploader') or 'YouTube'], year=e.get('release_date'), explicit=False, duration=e.get('duration'), additional=[e.get('webpage_url')], extra_kwargs={'webpage_url': e.get('webpage_url')})
            results.append(sr)
        return results
    ...
    def get_track_info(self, track_id, quality_tier, codec_options, **kwargs):
        url = track_id if str(track_id).startswith('http') else f'https://www.youtube.com/watch?v={track_id}'
    ...
    def get_track_download(self, webpage_url=None, **kwargs):
        ...
        # choose best by abr then filesize
    ...
    def get_playlist_info(self, playlist_id, **kwargs):
        url = playlist_id if str(playlist_id).startswith('http') else f'https://www.youtube.com/playlist?list={playlist_id}'
        ...
        pi = PlaylistInfo(name=title, creator=data.get('uploader') or 'YouTube', tracks=tracks, release_year=0, duration=sum((e.get('duration') or 0) for e in entries), explicit=False)
        return pi
