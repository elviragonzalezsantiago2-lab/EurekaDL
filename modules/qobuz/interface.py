...
module_information = ModuleInformation(
    ...
    session_settings={},
    flags=ModuleFlags(0),
    netlocation_constant=['qobuz.com']
)
...
class ModuleInterface:
    ...
    def search(self, media_type, query, limit=10, track_info=None):
        ...
        results = []
        for e in entries:
            sr = SearchResult(result_id=e.get('id') or e.get('url'), name=e.get('title'), artists=[e.get('uploader') or 'Qobuz'], year=e.get('release_date'), explicit=False, duration=e.get('duration'), additional=[e.get('webpage_url')], extra_kwargs={'webpage_url': e.get('webpage_url')})
            results.append(sr)
        return results
    ...
    def get_track_info(self, track_id, quality_tier, codec_options, **kwargs):
        url = track_id if str(track_id).startswith('http') else f'https://www.qobuz.com/track/{track_id}'
        info = self._run_yt_dlp_json(url)
        title = info.get('title')
        uploader = info.get('uploader') or 'Qobuz'
    ...
    def get_playlist_info(self, playlist_id, **kwargs):
        url = playlist_id if str(playlist_id).startswith('http') else f'https://www.qobuz.com/playlist/{playlist_id}'
        data = self._run_yt_dlp_json(url)
        title = data.get('title') or 'Qobuz Playlist'
        entries = data.get('entries') or []
        tracks = [e.get('webpage_url') for e in entries if e.get('webpage_url')]
        pi = PlaylistInfo(name=title, creator=data.get('uploader') or 'Qobuz', tracks=tracks, release_year=0, duration=sum((e.get('duration') or 0) for e in entries), explicit=False)
        return pi
