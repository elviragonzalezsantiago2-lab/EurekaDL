...
module_information = ModuleInformation(
    ...
    netlocation_constant=['bandcamp.com']
)
...
class ModuleInterface:
    ...
    def get_track_info(self, track_id, quality_tier, codec_options, **kwargs):
        url = track_id if str(track_id).startswith('http') else str(track_id)
        data = self._run_yt_dlp_json(url)
        e = data if isinstance(data, dict) else {}
        return TrackInfo(name=e.get('title') or str(track_id), album=e.get('album') or '', album_id=e.get('album_id') or '', artists=[e.get('uploader') or 'Bandcamp'], tags=Tags(), codec=CodecEnum.NONE, cover_url=e.get('thumbnail') or '', release_year=0, duration=e.get('duration') or 0, download_extra_kwargs={'webpage_url': e.get('webpage_url') or url})
    ...
    def get_playlist_info(self, playlist_id, **kwargs):
        url = playlist_id if str(playlist_id).startswith('http') else str(playlist_id)
        data = self._run_yt_dlp_json(url)
        title = data.get('title') or 'Playlist'
        entries = data.get('entries') or []
        tracks = [e.get('webpage_url') for e in entries if e.get('webpage_url')]
        pi = PlaylistInfo(name=title, creator=data.get('uploader') or 'Bandcamp', tracks=tracks, release_year=0, duration=sum((e.get('duration') or 0) for e in entries), explicit=False)
        return pi
